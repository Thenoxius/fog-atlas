...
8. **Initiative tracker**: click **Initiative** in the toolbar to open the panel. Flag your players' characters with the shield button in the **Characters** roster and **Add party** seats the whole group at once (never duplicated, even if some are already in). Add other combatants by hand with a name and initiative score, or click **From roster** to drop in a saved character — pick one and it's added at initiative 0 with its portrait, the roll field focused so you can type the roll straight away. Roster enemies auto-number as you add them (Goblin #1, Goblin #2…), derived from the current encounter so the count resets when you clear it; a roster enemy with a saved max HP starts at full. Combatants are auto-sorted highest first, and **Next Turn** advances through the order and counts rounds. Expand any row (click its portrait/chevron) for a DM-only detail card: the linked character's stat block (AC, HP, speed, ability scores with modifiers, notes) and a condition tracker with one-tap buttons for the common conditions plus a free-text field; give a condition an optional duration in rounds and it counts down on its own, clearing when the combatant's turn ends the right number of times. Active conditions show as small badges on the collapsed row. The skull button marks a combatant **down/dead** — greyed out, struck through, and skipped by Next Turn (press the skull again to bring them back up). Under **Encounters** you can save the current fight as a prepped encounter ("Goblin ambush at the bridge") and load it back with one click at the session — enemies are re-numbered fresh, HP starts full, and PCs already seated aren't duplicated. HP, stats, and conditions are all for your reference only — they're never sent to the player screen; the down state *is* shown to players (they saw it drop) as a greyed-out chip. The encounter is global (not tied to one map), survives page reloads, and its turn order streams live to the player screen as a compact bar with the current turn highlighted — just names and portraits, no HP (the portrait image is read from your local storage on the player side, never sent over the link).
